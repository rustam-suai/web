    ...
    <div class="page">
        <div class="block0">
            <ul class="nav">
                ...
                <li><a class="mainbuttons" href="series.php">Серии</font></a></li>
        ...
        <div class="container">
            ...
            <div class="block2">
                ...
                if(isset($_POST['add_to_list'])){
                    ...
                    $user_login = intval($_SESSION['id_user']);
                    //тут остановился. надо сделать добавление в "корзину"

                    // prepare the query to insert data into the table
                    $query_check = "SELECT id FROM busket WHERE (id_ser = $ser_id AND id_user = $user_login)";
                    $re_check = mysqli_query($conn, $query_check);
                    $rowi = mysqli_fetch_array($re_check);
                    if (empty($rowi)){
                        $query = "INSERT INTO busket (id_ser, id_user) VALUES ('$ser_id', '$user_login')";
                        ...
                        // check if the query was successful
                        if($result) {
                            echo '<p>Серия добавлена.</p>';
                        } else {
                            echo '<p>Ошибка при добавлении серии: ' . mysqli_error($conn) . '</p>';
                        }
                    }
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    if ($_SESSION['user'] == "in"){
                        ...
                        if(empty($row4['id'])){
                            echo '<td><form method="POST" action=""><input type="hidden" name="id" value="' . $row['id'] . '"><input type="submit" name="add_to_list" value="+"></form></td>';
                        }
                        else{
